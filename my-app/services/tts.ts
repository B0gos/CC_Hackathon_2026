...
const apiKey =
  Constants.expoConfig?.extra?.elevenlabsApiKey as string | undefined;
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onloadend = () => {
      if (typeof reader.result !== 'string') {
        reject(new Error('Unable to convert audio blob.'));
        return;
      }
...
async function cleanupCurrentSound(): Promise<void> {
  ...
  if (currentUri) {
    try { await FileSystem.deleteAsync(currentUri, { idempotent: true }); } catch {}
    currentUri = null;
  }
}
